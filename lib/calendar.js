...
Calendar.prototype = {
    ...
    monthDates : function monthDates(year, month, dayFormatter, weekFormatter) {
        ...
        if ((typeof month !== "number") || (month < 1) || (month > 12)) {
            throw new CalendarException('month must be a number');
        };
        var weeks = [],
            week = [],
            i = 0,
            date = this.weekStartDate(new Date(year, month-1, 1));
        do {
            for (i=0; i<7; i++) {
    ...
    monthDays : function monthDays(year, month) {
        var getDayOrZero = function getDayOrZero(date) {
            return date.getMonth() + 1 === month ? date.getDate() : 0;
        };
    ...
    monthText : function monthText(year, month) {
        ...
        var getDayOrBlank = function getDayOrBlank(date) {
            var s = date.getMonth() + 1 === month ? date.getDate().toString() : "  ";
